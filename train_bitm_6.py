...
from models.bitm import BiTMLLaDAStrictAR
...
##### ---- LLaDA Tokenizer ---- #####
llada_name = getattr(args, 'llada_name', 'GSAI-ML/LLaDA-8B-Base')
llada_config = AutoConfig.from_pretrained(llada_name, trust_remote_code=True)
base_tokenizer = AutoTokenizer.from_pretrained(llada_name, trust_remote_code=True)


class CausalTextTokenizerWrapper:
    def __init__(self, tokenizer, bos_id, eos_id, pad_id):
        self.tokenizer = tokenizer
        self.bos_token_id = bos_id
        self.cls_token_id = bos_id
        self.eos_token_id = eos_id
        self.sep_token_id = eos_id
    ...
    def __call__(self, texts, padding='max_length', truncation=True, max_length=None, return_tensors=None, **kwargs):
        if isinstance(texts, str):
            texts = [texts]
        texts = list(texts)

        if padding != 'max_length':
            raise ValueError("CausalTextTokenizerWrapper only supports padding='max_length'.")
        if max_length is None:
            raise ValueError("max_length must be provided.")
        if max_length < 2:
            raise ValueError("max_length must leave room for BOS and EOS.")

        payload_max_len = max_length - 2
        ...
        for ids in encoded['input_ids']:
            ids = [self.bos_token_id] + ids[:payload_max_len] + [self.eos_token_id]
            mask = [1] * len(ids)
        ...
        if return_tensors != 'pt':
            raise ValueError("CausalTextTokenizerWrapper only supports return_tensors='pt' or None.")
        return {
...
mask_id_t = getattr(llada_config, 'mask_token_id', None)
if mask_id_t is None:
    mask_id_t = base_tokenizer.mask_token_id
if mask_id_t is None:
    mask_id_t = base_tokenizer.convert_tokens_to_ids('<|mdm_mask|>')

bos_id_t = base_tokenizer.bos_token_id if base_tokenizer.bos_token_id is not None else base_tokenizer.cls_token_id
eos_id_t = base_tokenizer.eos_token_id if base_tokenizer.eos_token_id is not None else getattr(llada_config, 'eos_token_id')
pad_id_t = base_tokenizer.pad_token_id if base_tokenizer.pad_token_id is not None else eos_id_t
if bos_id_t is None or eos_id_t is None or pad_id_t is None or mask_id_t is None:
    raise ValueError("Could not resolve LLaDA BOS/EOS/PAD/MASK token ids.")

tokenizer = CausalTextTokenizerWrapper(base_tokenizer, bos_id_t, eos_id_t, pad_id_t)
special_ids_t = {
    'mask_id': mask_id_t,
    'cls_id': bos_id_t,
    'eos_id': eos_id_t,
...
bitm_model = BiTMLLaDAStrictAR(llada_name=llada_name,
                               vqvae=net,
...
logger.info(f'LLaDA backbone: {llada_name}.')
logger.info(f'LLaDA text ids: bos={special_ids_t["cls_id"]}, eos={special_ids_t["eos_id"]}, '
            f'pad={special_ids_t["pad_id"]}, mask={special_ids_t["mask_id"]}.')
logger.info(
    f"Gradient accumulation: micro_batch_size={args.batch_size}, "
    f"grad_accum_steps={args.grad_accum_steps}, "
    f"effective_batch_size={args.batch_size * args.grad_accum_steps}. "
    f"total_iter/eval_iter/print_iter are counted in optimizer updates."
)
logger.info(
...
def train(mask_probs_m):
    ...
    for nb_iter in tqdm(range(1, args.total_iter + 1), position=0, leave=True):
        do_log = nb_iter % args.print_iter == 0
        log_sums = {
            'loss_m_masked': 0.0,
            'loss_m_unmasked': 0.0,
            'loss_m': 0.0,
            'loss_t': 0.0,
            'loss': 0.0,
            'acc_every_motion': 0.0,
            'acc_text_ar': 0.0,
            'acc_masked_motion': 0.0,
            'acc_no_masked_motion': 0.0,
        }
        ...
        for accum_idx in range(args.grad_accum_steps):
            ...
            if do_log:
                with torch.no_grad():
                    ...
                    log_sums['loss_m_masked'] += float(loss_m_masked.detach().cpu())
                    log_sums['loss_m_unmasked'] += float(loss_m_unmasked.detach().cpu())
                    log_sums['loss_m'] += float(loss_m.detach().cpu())
                    log_sums['loss_t'] += float(loss_t.detach().cpu())
                    log_sums['loss'] += float(loss.detach().cpu())
                    log_sums['acc_every_motion'] += float((right_masked_m * 100 / seq_mask_no_end_m.sum()).detach().cpu())
                    log_sums['acc_text_ar'] += float(get_ar_text_acc(logits['logits_t'], token_ids_t, seq_mask_t).detach().cpu())
                    log_sums['acc_masked_motion'] += float(get_acc(logits['logits_m'], token_ids_m, mask_token_m).detach().cpu())
                    log_sums['acc_no_masked_motion'] += float(
                        get_acc(logits['logits_m'], token_ids_m, no_mask_token_m).detach().cpu()
                    )
        ...
        if do_log:
            log_values = {key: value / args.grad_accum_steps for key, value in log_sums.items()}

            # [INFO] log loss
            writer.add_scalar('./Loss/motion_masked', log_values['loss_m_masked'], nb_iter)
            writer.add_scalar('./Loss/motion_unmasked', log_values['loss_m_unmasked'], nb_iter)

            writer.add_scalar('./Loss/motion', log_values['loss_m'], nb_iter)
            writer.add_scalar('./Loss/text_ar', log_values['loss_t'], nb_iter)
            writer.add_scalar('./Loss/all', log_values['loss'], nb_iter)

            # [INFO] log accuracy
            writer.add_scalar('./ACC/every_motion', log_values['acc_every_motion'], nb_iter)
            writer.add_scalar('./ACC/text_ar', log_values['acc_text_ar'], nb_iter)

            # [INFO] log mask/nomask
            writer.add_scalar('./ACC/masked_motion', log_values['acc_masked_motion'], nb_iter)
            writer.add_scalar('./ACC/no_masked_motion', log_values['acc_no_masked_motion'], nb_iter)
        ...
        if nb_iter == 0 or nb_iter % args.eval_iter == 0 or nb_iter == args.total_iter:
            ...
            if nb_iter == args.total_iter:
                ...
                data_loader = dataset_TM_eval.DATALoaderNew(args.dataname, codebook_test_dir, w_vectorizer,
                                                            args.nb_code,
                                                            batch_size=args.eval_batch_size, is_test=True,
            ...
            # Validation
                ...
                data_loader = dataset_TM_eval.DATALoaderNew(args.dataname, codebook_val_dir, w_vectorizer, args.nb_code,
                                                            batch_size=args.eval_batch_size, is_test=False,
                                                            tokenizer_t=tokenizer,
                                                            max_t=args.max_t, return_all_captions=True)
            # T2M Evaluation
